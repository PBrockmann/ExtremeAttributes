$(document).ready(function() {
    ...
    indiceChart = dc.rowChart("#chart-indiceType");
    ...
    d3.csv("data/anomalous_index_sigma_scenario.csv", function(csv) {
        ...
        var yearDimension = filter.dimension(function(p) {
            ...
            indiceDimension = filter.dimension(function(p) {
                return p.Index;
            }),
	...
	var height = 200;
		...
		.dimension(regionDimension)
            ...
            indiceChart.width(200)
                .height(200)
		.dimension(indiceDimension)
                .group(indiceGroup)
            ...
            xAxis_indiceChart = indiceChart.xAxis().ticks(4);
    ...
    function plotColourbar(colourDomain_array, colourRange_array) {
        ...
        g = svg.append("g").attr("transform", "translate(10,10)").classed("colorbar", true),
            ...
            .size(80).lineWidth(50).precision(1);
        g.call(cb);
    }

    function showTimeSeries(regionName) {
        //only show if ONE index filter has been selected
        if (indiceChart.filters().length == 1) {
            console.log("In showTimeSeries for ", regionName);
            //console.log("indiceChart.hasFilter(CDD): ", indiceChart.hasFilter("CDD"))
